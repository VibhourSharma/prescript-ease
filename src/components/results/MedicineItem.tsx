...
import { ChevronDown, ChevronUp, Copy, AlertCircle, Pill, Check, RefreshCw } from 'lucide-react';
...
const MedicineItem: React.FC<MedicineItemProps> = ({ medicine, index }) => {
    ...
    >
      ...
      {isOpen && (
        <div className="p-4 animate-slide-down">
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{medicine.details.purpose}</p>
                </div>
                
                <div>
                  <h5 className="text-sm font-medium">Possible Side Effects</h5>
                  <p className="text-sm text-muted-foreground">{medicine.details.sideEffects}</p>
                </div>
                
                <div>
                  <h5 className="text-sm font-medium">Warnings</h5>
                  <p className="text-sm text-muted-foreground">{medicine.details.warnings}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-medium mb-2">Alternative Medications</h4>
          ...
          <div className="mt-4 flex items-start gap-2 bg-yellow-50 border border-yellow-200 p-3 rounded-md">
            <AlertCircle className="h-5 w-5 text-yellow-500 shrink-0 mt-0.5" />
            <div className="text-xs text-yellow-800">
              This information is for educational purposes only. Always consult your doctor or pharmacist for specific advice about your medication.
            </div>
          </div>
        </div>
      )}
